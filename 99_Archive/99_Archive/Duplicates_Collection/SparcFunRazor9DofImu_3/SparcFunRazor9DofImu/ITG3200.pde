...
void Init_Gyro()
{
  ...
  Wire.send(9);  // Sample rate divider is 10 (register value + 1)
  ...
  Wire.send( (0x03<<3) | (0x03<<0) );  // Set the full-scale range to +/- 2000deg/sec, and the low pass filter to 20Hz
  Wire.endTransmission();
  delay(20);	
  ...
  if (i==8)  // All bytes received?
    {
      ...
      if(sen_data.accel_x<0){
        sen_data.gyro_x *= 0.99;
      }

      if(sen_data.accel_y<0){
        sen_data.gyro_y *= 0.85 ;
      }
      sen_data.gyro_y *= 1.03;

      if(sen_data.accel_z!=0){
        sen_data.gyro_z *= 0.9;
      }
      
      if(abs(sen_data.gyro_x)<0.02){
        sen_data.gyro_x = 0.0;
      }
      
      if(abs(sen_data.gyro_y)<0.02){
        sen_data.gyro_y = 0.0;
      }
      
      if(abs(sen_data.gyro_z)<0.02){
        sen_data.gyro_z = 0.0;
      }
    }
  else
