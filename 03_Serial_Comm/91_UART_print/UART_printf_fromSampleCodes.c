...
void uart_printf(unsigned char data, FILE * stream) {
//void putch(unsigned char data) {
	//전송준비가 될 때까지 대기
	while ((UCSR0A & (1 << UDRE0)) == 0)
		;
...
static FILE PrnDevice = FDEV_SETUP_STREAM(uart_printf, NULL, _FDEV_SETUP_WRITE);
static FILE KeyDevice = FDEV_SETUP_STREAM(NULL, uart_scanf, _FDEV_SETUP_READ);
